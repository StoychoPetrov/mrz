...
class Camera2BasicFragment : Fragment(), View.OnClickListener,
    ...
    private lateinit var tesseractOCR: TesseractOCR2
    ...
    private val onImageAvailableListener = ImageReader.OnImageAvailableListener {
        ...
        if(image != null) {
            ...
            if(planes.size == 3) {
                ...
                if (bitmap != null) {
                    Log.d("Crop sucess", "Success")
                    ocr(bitmap)
                }
                else
                    Log.d("Crop sucess", "Error")
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                if(displayRotation != null) {
                    ...
                    (activity as FragmentActivity).windowManager.defaultDisplay.getSize(displaySize)
    ...
    private fun cropCameraViewFrame(cvCameraViewFrame: Mat): Bitmap? {
        val cropResult = DetectionBasedTracker.detectPassportZone(cvCameraViewFrame.nativeObjAddr)

        if(cropResult != null)
            return BitmapFactory.decodeByteArray(cropResult, 0, cropResult.size)
    ...
    private fun openCamera(width: Int, height: Int) {
        if(activity != null) {
            ...
            configureTransform(width, height)
            val manager = (activity as FragmentActivity).getSystemService(Context.CAMERA_SERVICE) as CameraManager
            try {
    ...
    private fun createCameraPreviewSession() {
        try {
            ...
            previewRequestBuilder.addTarget((imageReader as ImageReader).surface)
